...
namespace Crunchy
{
	...
	{
		private static CultureInfo _enUSCultureInfo = null;

		static StringTools()
		{
			_enUSCultureInfo = CultureInfo.CreateSpecificCulture("en-US");
		}
		...
		public static bool TryConvertToString(Type objectType, object valueObject, out string valueString)
		{
			...
			try
			{
				...
				valueString = typeConverter.ConvertToString(null, _enUSCultureInfo, valueObject);
			}
			catch
			{
				return false;
		...
		public static bool TryConvertFromString(Type objectType, string valueString, out object valueObject)
		{
			...
			else if (objectType == typeof(float))
			{
				...
				if (float.TryParse(valueString, NumberStyles.Any, _enUSCultureInfo, out valueFloat))
				{
			...
			else if (objectType == typeof(double))
			{
				...
				if (double.TryParse(valueString, NumberStyles.Any, _enUSCultureInfo, out valueDouble))
				{
			...
			{
				TypeConverter typeConverter = TypeDescriptor.GetConverter(objectType);
				valueObject = typeConverter.ConvertFromString(null, _enUSCultureInfo, valueString);
			}
			catch
			{
				return false;
